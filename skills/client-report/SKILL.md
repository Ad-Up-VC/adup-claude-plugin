...
2b. **Already drafted by the managed Reporting Skill?** Call
   ...
   *Open* (or no answer) → call `get_agent_output(run_id="<id>", kind="html")` and print the
   ...
   submission will sit next to the agent's draft in the review queue. The tool being absent
   (older gateway), erroring, or returning no `ready` run → continue silently; never hard-fail
   the report over it. Never call `run_agent` from this skill.
...
Pull data from EVERY connected platform. Every data tool is **namespaced `platform__tool`** and takes an explicit `shop_slug` on every call. (Only the virtual tools — `list_shops`, `set_active_shop`, `create_report`, `get_kpi`, `get_report_template`, `get_report_branding`, and the managed-agent tools `list_agents` / `get_agent_runs` / `get_agent_output` — are unprefixed.)
...
- [ ] `get_agent_runs` checked for a `ready` managed-agent draft covering the period; if one existed, the user was offered it (default: open in Tara) before anything was built
...
13. **One draft per period, one voice.** The managed Reporting Skill in Tara files into the same
    ...
    sit *after* the contract and these rules, never above them. Never start an agent run from
    here — that is `/adup:agents run`, with its own confirmation.
