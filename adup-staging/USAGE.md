...
| Skills (`/adup-staging:facebook-ads`, …) | ✅ 26 bundled | ❌ none — a plugin feature |
...
```
/adup-staging:setup      # stores your API key
/adup-staging:connect    # verifies the key, environment, and which brands you can see
```
...
| | |
|---|---|
| URL | `https://gateway-staging.adup.io/mcp` |
...
- `https://gateway-staging.adup.io/mcp/custom` — external MCP servers your organisation registered in
  the Control Center. These are **not** part of the base connector.
- `https://gateway-staging.adup.io/mcp/{platform}` — the legacy per-platform routes. Still supported,
  no reason to use them for new setups.

### Environments

Production needs no configuration. If you are on an internal environment, set
`ADUP_STAGING_GATEWAY_BASE` **and** `ADUP_STAGING_API_BASE` together — the gateway serves the MCP tools,
