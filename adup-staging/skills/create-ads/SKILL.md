...
> **Launching multiple ads, working from local creative files, or going cross-platform (Facebook + TikTok)?** Use the creative workspace skill family instead: `/adup-staging:creative-workspace init` to set up a local workspace, `/adup-staging:launch` to validate/upload/propose in bulk, `/adup-staging:status` to sync approval state back, `/adup-staging:creative-import` to import winners or spreadsheets. This skill remains the quick path for creating a **single ad into an existing ad set** conversationally.
