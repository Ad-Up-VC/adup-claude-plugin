...
Modes:
- `/adup-staging:creative-workspace init [folder]` — scaffold a new workspace
- `/adup-staging:creative-workspace doctor [folder]` — structural health check (report only, never modifies content)
- `/adup-staging:creative-workspace doctor --grouping [folder]` — grouping preview: show how assets would be grouped and mapped at launch, nothing else
...
- Default platforms for new ads (e.g. `["facebook", "tiktok"]`). Creation is supported for facebook, tiktok, google (RSA text ads only — no media), and linkedin (single image/video ads); snapchat is **not yet supported for creation** — it can be listed but /adup-staging:launch will skip it with a note.
...
map:                                 # WHERE ads land, per platform. v1: use EXISTING ids.
                                     ...
                                     # doesn't target yet: /adup-staging:launch embeds all of them as
...
> Drop media into `assets/<concept>/` — **any file names work**, I detect each file's ratio and format from its actual pixels. Write ads as `ad.md` files, then run `/adup-staging:launch`. Everything I launch becomes a **proposal** for approval in the ADUP portal, and approved ads always start **PAUSED**.
...
8. **Lifecycle sanity** — `status: proposed|approved|live` ads exist in `state.json` with proposal ids (otherwise state was lost — suggest re-running /adup-staging:launch, which is idempotent by checksum).
