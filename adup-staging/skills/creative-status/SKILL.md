...
# Creative Status (/adup-staging:status)
...
Usage: `/adup-staging:status [path] [--csv] [--sheet]`
...
2. Collect every `targets` entry across `state.json` `ads`: `(ad_path, platform, lang, proposal_id, last_known_status)`. Nothing recorded → say "nothing launched yet — run /adup-staging:launch" and stop.

## Step 2 — Query proposal statuses

Query central-api's **employee** proposals surface with the employee key (same Bearer as the MCP connector) — the same `ADUP_STAGING_API_BASE` used for replication-requests below. List per shop, paginated:

```bash
curl -s -H "Authorization: Bearer $ADUP_STAGING_API_KEY" -H "Accept: application/json" \
  "${ADUP_STAGING_API_BASE:-https://centralapi-staging.adup.io}/api/v2/employee/tara/proposals?shop_slug=<shop_slug>&per_page=100&page=1"
```
...
```bash
curl -s -H "Authorization: Bearer $ADUP_STAGING_API_KEY" -H "Accept: application/json" \
  "${ADUP_STAGING_API_BASE:-https://centralapi-staging.adup.io}/api/v2/employee/tara/proposals/<proposal_id>"
```
...
3. **Denial notes**: when a proposal was denied with a reviewer note, set the ad's `status: changes_requested` and append (or update) a `## Review feedback` section at the bottom of that `ad.md`:
   ...
   Never overwrite existing feedback entries; append new ones. After the user edits the copy, `/adup-staging:launch` picks the ad up again (content hash changed) and re-proposes it.
...
```bash
curl -s -H "Authorization: Bearer $ADUP_STAGING_API_KEY" -H "Accept: application/json" \
  "${ADUP_STAGING_API_BASE:-https://centralapi-staging.adup.io}/api/v2/employee/tara/actions/replication-requests?shop_slug=<shop_slug>&status=pending"
```
...
4. **Launch**: with the user's go-ahead, run the normal `/adup-staging:launch` flow for that ad scoped to the requested platform (validate → upload → propose; same batch/count-confirmation rules). The proposal calls are namespaced `platform__tool` and each carries the workspace's `shop_slug="<slug>"` explicitly — a replication write must never land on another client.
5. **Close the request**:

   ```bash
   curl -s -X PATCH -H "Authorization: Bearer $ADUP_STAGING_API_KEY" -H "Content-Type: application/json" -H "Accept: application/json" \
     -d '{"status": "fulfilled", "fulfilled_proposal_id": "<new proposal id>"}' \
     "${ADUP_STAGING_API_BASE:-https://centralapi-staging.adup.io}/api/v2/employee/tara/actions/replication-requests/<id>"
   ```
...
1. **One-way sync for statuses**: platform state → files. This skill never approves or denies proposals, and the only proposals it ever creates are replication-request fulfilments (Step 4) — explicitly user-confirmed and routed through the normal `/adup-staging:launch` flow and approval queue.
