...
```bash
echo "gateway:     ${ADUP_STAGING_GATEWAY_BASE:-https://gateway-staging.adup.io  (production default)}"
echo "central-api: ${ADUP_STAGING_API_BASE:-https://centralapi-staging.adup.io  (production default)}"
```
...
Mention a client name to switch to it, or run /adup-staging:shop-select.
```
...
If your agency owner has installed additional skills (e.g., "LinkedIn Engagement Analysis") in the ADUP portal, run `/adup-staging:sync-skills` to pull them into your local Claude. They'll be available after a Claude restart.
...
"The ADUP connection needs authentication. Set your personal API key: `export ADUP_STAGING_API_KEY=your_key_here`, then restart Claude Code. If you don't have a key, ask your agency owner to invite you in the portal."
...
by default and returns `invalid_token` while being perfectly valid. Run `/adup-staging:setup` and pick the
matching environment at Step 2b.

If the MCP server itself is unreachable (connection refused or timeout):
"Cannot reach the ADUP gateway at `${ADUP_STAGING_GATEWAY_BASE:-https://gateway-staging.adup.io}/mcp`. Check that your ADUP_STAGING_API_KEY is set correctly and the service is running."

Report the **resolved** host, not the production default — if `ADUP_STAGING_GATEWAY_BASE` is set, an
