...
# Creative Launch (/adup-staging:launch)
...
Usage: `/adup-staging:launch [path]` — path may be a workspace root, a campaign folder, an ad set folder, or a single ad folder. No path = current directory's workspace, all campaigns.

Environment: `ADUP_STAGING_API_KEY` (personal employee key), API base `${ADUP_STAGING_API_BASE:-https://centralapi-staging.adup.io}`.

---

## Step 1 — Resolve context

1. Find the workspace root: walk up from the given path until a directory containing `.adup/workspace.json` is found. Missing → tell the user to run `/adup-staging:creative-workspace init` first and stop.
...
2. Otherwise run:
   ```bash
   bash <plugin>/skills/creative-workspace/scripts/upload.sh "${ADUP_STAGING_API_BASE:-https://centralapi-staging.adup.io}" <shop_slug> "<file>" "<campaign-slug>,<concept>"
   ```
...
```bash
curl -s -X POST -H "Authorization: Bearer $ADUP_STAGING_API_KEY" -H "Content-Type: application/json" -H "Accept: application/json" \
  -d '{"url": "<share-link>", "filename": "<concept>_<ratio>.<ext>", "tags": ["<campaign-slug>"]}' \
  "${ADUP_STAGING_API_BASE:-https://centralapi-staging.adup.io}/api/v1/shops/<shop_slug>/creative-assets/from-url"
```
...
- **`map.<platform>.create: true`** (new campaign/ad set) is **two-phase** in v1 for platforms that expose an approval-gated parent-creation tool: propose the campaign/ad set creation first (`tiktok__propose_create_campaign` / `tiktok__propose_create_adgroup`, each with `shop_slug` and `batch_id`), then tell the user to approve those in the portal and run `/adup-staging:status` to capture the created ids into `map:` — THEN re-run `/adup-staging:launch` for the ads. Do not chain unresolved parent ids in one batch.
  - **Facebook has no approval-gated campaign/ad-set creation tool** (the only `facebook__*` create paths that would make a campaign or ad set are the direct `ads_campaign_create` / `ads_adset_create` tools, which bypass the approval queue — never call them here). So `map.facebook.create: true` is **not supported for parent creation in v1**: tell the user to create the campaign/ad set in Ads Manager (or the ADUP portal) first and put the resulting `campaign_id` / `adset_id` into `map.facebook`, then launch the ads (which DO go through the queue). This mirrors the `/adup-staging:create-ads` rule — new ads land in existing ad sets only.
...
**Facebook** (per ad × language) — **always through the approval middleware; never a direct `facebook__ads_*_create` tool** (those create on the platform immediately and bypass the queue). Facebook's proposal tools are `facebook__propose_create_ad` (single) and `facebook__propose_bulk_launch` (batch) — the same pair every other platform is held to.
- **Preferred path — `facebook__propose_bulk_launch`** (also the >3-ads path above; use it even for a single ad here). It is the only Facebook proposal tool whose per-ad spec accepts the workspace's uploaded `asset_id` / `video_id`, a carousel `child_attachments` array, `enhancements_opt_out`, and a per-item `metadata.platform_targets`. Pass `shop_slug="<slug>"`, `batch_id`, `shared_reasoning`, and an `ads[]` array where each element carries: `adset_id` (from map), `ad_name` (`<ad-folder> | <lang>` or the account's convention), `page_id`, the creative spec (`asset_id`/`video_id`/`image_hash` or `child_attachments`), resolved primary text/headline/description (`message`/`title`/`description`), `link_url`, `call_to_action`, `enhancements_opt_out`, and `metadata` with the `platform_targets` object above.
- **Simple path — `facebook__propose_create_ad`** (≤3 ads, no uploaded-asset/carousel/platform_targets requirement): pass `shop_slug="<slug>"` (always explicit, on every proposal call below too), `adset_id` (from map), `ad_name`, `reasoning` (one sentence: campaign + why), and either an existing `creative_id` **or** the new-creative fields `page_id` + `message` + `link` + `headline` + `call_to_action` (+ `image_hash` for an already-uploaded image). `propose_create_ad` does not carry `asset_id`, `batch_id`, or `metadata` — if you need any of those, use `facebook__propose_bulk_launch` (a single-element batch is fine).
- **No standalone library creative in v1.** There is no approval-gated tool for creating a reusable creative on its own — do **not** call `facebook__ads_creative_create` (it bypasses the queue). Fold the creative into the ad proposal instead; both propose tools embed the full creative spec inline.
Every Facebook proposal files to the approval queue — nothing is created on Meta until a human approves it in the portal, and approved ads land PAUSED.
