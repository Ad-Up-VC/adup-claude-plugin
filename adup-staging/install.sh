...
export ADUP_STAGING_API_KEY="$API_KEY"

echo -e "${YELLOW}Step 1/3:${NC} Saving API key for this user ($USER)..."
echo "  ADUP_STAGING_API_KEY=****${API_KEY: -4}"
...
#
# The plist runs `launchctl setenv ADUP_STAGING_API_KEY <value>` at login, making the
...
PLIST_FILE="$LAUNCH_AGENTS_DIR/io.adup.env.ADUP_STAGING_API_KEY.plist"
...
<dict>
    <key>Label</key>
    <string>io.adup.env.ADUP_STAGING_API_KEY</string>
    <key>ProgramArguments</key>
    <array>
        <string>launchctl</string>
        <string>setenv</string>
        <string>ADUP_STAGING_API_KEY</string>
...
launchctl setenv ADUP_STAGING_API_KEY "$API_KEY"

echo -e "  ${GREEN}✓${NC} API key registered as macOS LaunchAgent — available to all desktop apps for this user"
echo -e "     Saved to: ~/Library/LaunchAgents/io.adup.env.ADUP_STAGING_API_KEY.plist"
...
settings["env"]["ADUP_STAGING_API_KEY"] = api_key
...
if [ -n "$SHELL_PROFILE" ]; then
  grep -v "export ADUP_STAGING_API_KEY=" "$SHELL_PROFILE" > "$SHELL_PROFILE.tmp" 2>/dev/null || true
  mv "$SHELL_PROFILE.tmp" "$SHELL_PROFILE"
  echo "export ADUP_STAGING_API_KEY=\"$API_KEY\"" >> "$SHELL_PROFILE"
