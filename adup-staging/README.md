...
|---|---|
| Name | `adup` |
| URL | `${ADUP_STAGING_GATEWAY_BASE:-https://gateway-staging.adup.io}/mcp` |
...
no per-platform connectors to add. `/adup-staging:setup` writes `ADUP_STAGING_API_KEY` for you; if
you set it by hand, `export ADUP_STAGING_API_KEY=your_key_here` and restart Claude Code.
...
export ADUP_STAGING_GATEWAY_BASE=https://gateway-staging.adup.io
export ADUP_STAGING_API_BASE=https://centralapi-staging.adup.io
...
`invalid_token` even though it is valid. `/adup-staging:connect` reports which environment you
...
|-------|---------|-------------|
| Setup | `/adup-staging:setup` | Configure your API key (run once after install) |
| Connect | `/adup-staging:connect` | Verify connection and see available shops |
| Shop Select | `/adup-staging:shop-select` | Switch active client (agencies) |
| Sync Skills | `/adup-staging:sync-skills` | Pull the skills your agency installed in the ADUP portal into your local Claude |

### Analysis
| Skill | Command | Description |
|-------|---------|-------------|
| Ads Overview | `/adup-staging:ads-overview` | Quick cross-platform ad summary |
| Facebook Ads | `/adup-staging:facebook-ads` | Facebook & Instagram analysis |
...
| Inspiration | `/adup-staging:inspiration` | "Next steps for this client" briefing — prioritised recommendations tied to KPI gaps |

### Actions (via middleware — all proposals require approval)
| Skill | Command | Description |
|-------|---------|-------------|
| Optimize Budget | `/adup-staging:optimize-budget` | Propose budget increases/decreases with data |
| Manage Status | `/adup-staging:manage-status` | Propose pausing/activating campaigns, ad sets, ads |
| Create Ads | `/adup-staging:create-ads` | Propose new ad creation within existing ad sets |
| Google Optimize | `/adup-staging:google-optimize` | Propose Google Ads budget, status & RSA changes |
| LinkedIn Optimize | `/adup-staging:linkedin-optimize` | Propose LinkedIn budget, status & creative changes |
| TikTok Optimize | `/adup-staging:tiktok-optimize` | Propose TikTok budget & status changes |

### Creative Workspace (bulk launching — all proposals require approval, ads land PAUSED)
| Skill | Command | Description |
|-------|---------|-------------|
| Creative Workspace | `/adup-staging:creative-workspace` | Init a local creative workspace (folders + markdown) or run doctor checks |
| Launch | `/adup-staging:launch` | Validate, upload, and propose ads in bulk per ad × platform × language |
| Status | `/adup-staging:status` | Sync approval statuses back into the workspace; `--csv` / `--sheet` exports |
| Creative Import | `/adup-staging:creative-import` | Import a winning ad for cross-platform relaunch, or a spreadsheet copy matrix |

### Reporting
| Skill | Command | Description |
|-------|---------|-------------|
| Monday Briefing | `/adup-staging:monday-briefing` | Per-client executive summary with wins & concerns |
| Client Report | `/adup-staging:client-report` | Client-ready report with talking points |
| Anomaly Alerts | `/adup-staging:anomaly-alerts` | Detect spend spikes, delivery stops, CTR drops |
